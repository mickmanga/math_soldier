...
(See https://redux.js.org/faq/organizing-state#can-i-put-functions-promises-or-other-non-serializable-items-in-my-store-state)`);
  ...
  var subblocs = document.querySelectorAll(".subbloc");
  var addBlocBtn = document.getElementById("addBlocBtn");
  var addSubblocBtns = document.querySelectorAll(".addSubblocBtn");
  window.store = store_default;
  var subblocCounter = 3;
  function addDragAndDropListeners(element) {
    element.addEventListener("dragstart", (event) => {
      var _a;
      const e = event;
      (_a = e.dataTransfer) == null ? void 0 : _a.setData("text/plain", e.target.id);
    });
    element.addEventListener("dragover", (event) => {
      event.preventDefault();
      const e = event;
      const target = e.target;
      if (target.classList.contains("subbloc") || target.classList.contains("bloc")) {
  ...
  blocs.forEach((bloc) => {
    ...
    addSubblocBtn == null ? void 0 : addSubblocBtn.addEventListener("click", () => {
      var _a;
    ...
    `;
      bloc.insertBefore(
        newSubbloc,
        (_a = bloc.querySelector("h2")) == null ? void 0 : _a.nextSibling
      );
      addDragAndDropListeners(newSubbloc);
      const arrow = newSubbloc.querySelector(".expand-arrow");
      addExpandListener(arrow);
      makeContentEditable(
        newSubbloc.querySelector(".subbloc-title")
      );
      makeContentEditable(
        newSubbloc.querySelector(".subbloc-body")
      );
    });
  });
  addBlocBtn == null ? void 0 : addBlocBtn.addEventListener("click", () => {
    ...
    if (board) {
      ...
      `;
        newBloc.insertBefore(
          newSubbloc,
          (_a = newBloc.querySelector("h2")) == null ? void 0 : _a.nextSibling
        );
        addDragAndDropListeners(newSubbloc);
        const arrow = newSubbloc.querySelector(".expand-arrow");
        addExpandListener(arrow);
